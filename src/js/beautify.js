var check_jsonp = function(js){
    var m = js.match(/^[A-Za-z0-9\._-]+/);
    if (m) {
        document.getElementById("jsonp_detected").innerText ="JSONPコールバック→ " + m[0] +"()";
    }
    return js.replace( /^[A-Za-z0-9\._-]+\(/g , "" ).replace(/\)$/,"");
}
var beautify_object = function () {
    document.getElementById("jsonp_detected").innerText ="";
    objtxt = document.getElementById("minified_json_text").value;
    try {
        js = check_jsonp(objtxt);
        js = JSON.parse(js);
        result = JSON.stringify(js, null, "   ");
        document.getElementById("beautified_json_text").value = result;
    } catch (E) {
        document.getElementById("beautified_json_text").value = E +"\nInputText↓\n" + objtxt;
    }
    
}

var beautify_xml = function(){
    objtxt = document.getElementById("minified_xml_text").value;
    try {
        var format = require('xml-formatter');
        document.getElementById("beautified_xml_text").value = format(objtxt);
    } catch (error) {
        document.getElementById("beautified_xml_text").value = error;
    }
}

/*
Copyright (c) 2016 chrisbottin
